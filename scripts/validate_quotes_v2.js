...
const FAILURE_PATH = path.resolve(
  "scripts",
  "_validation_failures_quotes_v2.json"
);
...
const validateRequestedItem = (item, index) => {
  ...
  if (
    ...
    errors.push(`requestedItems[${index}].productName must be a string if set.`);
  }

  if (item.priceRule != null && typeof item.priceRule !== "string") {
    errors.push(`requestedItems[${index}].priceRule must be a string if set.`);
  }
